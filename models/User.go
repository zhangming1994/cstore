...
*获取用户列表
 */
func GetUserList(start int64, length int64, sort string) (users []orm.Params, count int64) {
	omodel := orm.NewOrm()
	user := new(User)
	qs := omodel.QueryTable(user)

	qs.Limit(length, start).OrderBy(sort).Values(&users)
	count, _ = qs.Count()
	beego.Debug("=================users", users)
	return users, count
